...
app.get('/api/public/region/:code/org', (req, res) => {
    try {
        ...
        // Debug: Check if departments exist
        console.log('Departments available:', mockData.departments ? mockData.departments.length : 'undefined');
        if (mockData.departments) {
            console.log('Sample departments:', mockData.departments.slice(0, 3));
        }
        ...
        const formatUser = (u, officeName) => {
            ...
            if (u.departments && u.departments.length > 0) {
                departments = u.departments
                    .map(deptCode => {
                        const dept = mockData.departments.find(d => String(d.code) === String(deptCode));
                        if (dept) {
